const VacuumRobot = require('./vacuum_robot');
...
class IntcodeComputer {
  constructor(program, initialInput) {
    ...
    this.pointer = 0;
    this.relativeBase = 0;
    this.vacuumRobot = new VacuumRobot();
  }
  ...
  run() {
    ...
    while (this.pointer < program.length) {
      ...
      if (opCode === 1) {
        ...
        this.input = this.vacuumRobot.provideInstruction();
        console.log('Receiving input: ', this.input);
        program[writeAddress] = this.input;
        this.pointer += 2;
      } else if (opCode === 4) {
        console.log(param1);
        this.vacuumRobot.processInput(param1);
        this.pointer += 2;
      } else if (opCode === 5) {
